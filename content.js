...
function getFrequency(string) {
	var cleanString = string.replace(/[\d\.,-\/#!$%\^&\*;:{}=\-_`~()]/g, ""),
...
function getAllWords(string) {
	var frequencies = getFrequency(string);
...
function getTopWords(string, cutOff) {
	...
	return words.slice(0, cutOff).toString();
}

function test(content) {
	var ret = getTopWords(content, 100);
	alert(ret);
}
...
var readability = {
	...
	init : function () {
		...
		var allMarkNodes = document.querySelectorAll('body *');
		var markIdx = 0;
		for (markIdx = 0; markIdx < allMarkNodes.length; markIdx++) {
			var markNode = allMarkNodes[markIdx];
			markNode.className += " markIdx" + markIdx;
			//dbg('markIdx='+markIdx);
		}
		var originalBody = document.createElement('body');
    //		var originalBody2 = document.createElement('body');

		for (var i = 0; i < document.body.attributes.length; i++) {
			var attr = document.body.attributes.item(i);
			originalBody.setAttribute(attr.nodeName, attr.nodeValue);
	//		originalBody2.setAttribute(attr.nodeName, attr.nodeValue);
		}

		originalBody.innerHTML=document.body.innerHTML;
		//originalBody2.innerHTML=document.body.innerHTML;
		...
		var nodesBefore = document.querySelectorAll('body *').length;
		...
		var nodesAfter = innerDiv.querySelectorAll('body *').length;

		readStyle = "union-reader";
		document.body.className = readStyle;
		overlay.className = readStyle;
		wordCountDiv.className = "wordcount";
		innerDiv.className = readMargin + " " + readSize;

		/* Glue the structure of our document together. */
		// articleContent.appendChild( articleFooter  );
		innerDiv.appendChild(wordCountDiv);
...
/*    
       var titleMarkIdx = articleTitle.className;
		var selectedTitleNode = originalBody2.getElementsByClassName(titleMarkIdx)[0];

		var tempNode = selectedTitleNode;
		var titleParentChain = selectedTitleNode.parentNode;
		titleParentChain.innerHTML = "";
		titleParentChain.appendChild(selectedTitleNode);

		// Go until there are no more child nodes
		while (titleParentChain !== originalBody2) {
		tempNode=titleParentChain;
		titleParentChain=titleParentChain.parentNode;
		
    titleParentChain.innerHTML="";
    titleParentChain.appendChild(tempNode);
		}
    */
    
		/*1111111111111111*/
   
    var selectedNodes = articleContent.childNodes;
		var firstMarkIdx = selectedNodes[0].className;
		var firstSelectedNode = originalBody.getElementsByClassName(firstMarkIdx)[0];

		var tempNode = firstSelectedNode;
		var articleParentChain = firstSelectedNode.parentNode;
		articleParentChain.innerHTML = "";
		articleParentChain.appendChild(firstSelectedNode);

		//alert(firstSelectedNode.className);
		//alert(originalBody.className);
		
		// Go until there are no more child nodes
		while (articleParentChain !== originalBody) {
		tempNode=articleParentChain;
		articleParentChain=articleParentChain.parentNode;
		
    articleParentChain.innerHTML="";
    articleParentChain.appendChild(tempNode);
		}
		
    originalBody.innerHTML=articleParentChain.innerHTML;
		 document.body=originalBody;

		document.head.innerHTML = originalHead;
      document.body.insertBefore(articleTitle,document.body.firstChild);

    dbg(originalHead);
		/* word count */
		var regex = /\s+/gi;
		var wordCount = innerDiv.textContent.trim().replace(regex, ' ').split(' ').length;

		var readingTime = Math.ceil(wordCount / 230);

		wordCountDiv.innerHTML = /*
			"wordcount: "
			+wordCount+", estimated reading time: "
			+readingTime+" minutes, elements removed: "
			+(nodesBefore-nodesAfter)+", scripts removed: "
			+scriptsBefore; */
			"<div class='block'>"
			 + "<div class='number'>"
			 + wordCount
...
//		document.body.style.display = "block";
	...
	getArticleTitle : function () {
		...
		//alert('currentTitle '+curTitle.length);
	...
	prepArticle : function (articleContent) {
		...
		readability.clean(articleContent, "hr");
		/**
	...
	grabArticle : function () {
				...
				if (node.innerHTML.search(readability.regexps.divToPElementsRe) === -1) {
					...
					for (var i = 0, il = node.childNodes.length; i < il; i++) {
						...
						if (childNode.nodeType == 3) { // Node.TEXT_NODE
							...
							p.className = 'readability-styled originalTEXT';
		...
		dbg('++++++++++++++++++++++topCandidate: ' + topCandidate + " (" + topCandidate.className + ":" + topCandidate.id + ") with score " + topCandidate.readability.contentScore);
